...
test("execute command absorbs continue semantics", () => {
  ...
  assert.match(execute, /Model is fixed to gpt-5\.6-sol, the only supported --model value/i);
  ...
  assert.match(readme, /model is fixed to GPT 5\.6 sol; `gpt-5\.6-sol` \(alias `sol`\) is the only accepted --model value/i);
...
test("execute agent references the simplified recipes and no retired models", () => {
  const agent = read("agents/codex-execute.md");
  assert.match(agent, /bounded asks.*simplified prompt recipes/i);
  assert.doesNotMatch(agent, /gpt-5\.5|gpt-5\.4/);
});
