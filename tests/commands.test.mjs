...
test("execute command absorbs continue semantics", () => {
  ...
  assert.match(execute, /Model defaults to gpt-5\.6-sol and accepts --model <model>/i);
  ...
  assert.match(agent, /Model defaults to gpt-5\.6-sol and accepts --model <model>/i);
  ...
  assert.match(runtimeSkill, /Model defaults to gpt-5\.6-sol and accepts --model <model>/i);
  ...
  assert.match(readme, /model defaults to GPT 5\.6 sol \(also supports gpt-5\.5, gpt-5\.5-mini and gpt-5\.4\/5\.4-mini fallbacks via --model/i);
