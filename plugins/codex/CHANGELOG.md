...
- `--effort max` is now accepted (default stays `xhigh`); explicit model/effort pairs are validated against the app-server model catalog, which always runs and only no-ops on older CLIs without `model/list`
